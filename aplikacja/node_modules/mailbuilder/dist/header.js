 ...
 * @returns {string}
 */
export function escapeNameString(str) {
    if (!/^[\w ']*$/.test(str)) {
        if (/^[\x20-\x7e]*$/.test(str)) {
 ...
 */
export function shouldEncodedOrEscapeAddresses(addrArray) {
    return addrArray.some(({ name, address }) => (name && !isAscii(name)) || !isAscii(address));
}
/**
 ...
 * @returns {string}
 */
export function getAddressesHeader(addrArray) {
    return addrArray
        .map(({ name, address }) => name
        ? `${isAscii(name)
            ? escapeNameString(name)
            : utf8encode(name.normalize('NFC'))} <${address}>`
        : address)
        .join(', ');
}
/**
