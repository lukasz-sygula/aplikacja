...
describe('Test db interface', () => {
    ...
    test('It should insert data', async () => {
        try {
            const data = await db.insert('rss', { 'email':  'test2@test.pl', 'rss': 'rss.xml' })
    ...
    test('It should updata value', async () => {
        try {
            const res = await db.update('rss', {"email" : "test@test.pl" }, { $set: {'email': 'test@test.pl', 'rss': '2rss.xml' }})
            expect(res.result.ok).toBe(1)
        } catch (e) {
            throw(e)
        }
